...
CoinBaseTransaction::CoinBaseTransaction(uint64_t &id, std::string &pubkey): Transaction(id, 0, 1),
    m_address(pubkey){
...
RealTransaction::RealTransaction(const uint64_t &id, const std::string &from, const std::string &to, int value, std::list<TXInput> &inputs, int rest) :
